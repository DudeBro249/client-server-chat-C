...
// TODO: Put everything in main function so we can
// allow user to define maximum number of clients
        ...
        if(len > 0) {
            ...
            if (strcmp(recipient, "room") == 0) {
                ...
                if (client_found == 0) {
                    ...
                    char error_output[MAX_INPUT_OUTPUT_SIZE];
                    sprintf(error_output, "Could not find a client with ID %s", recipient);
                    write(client.socketfd, error_output, sizeof(error_output));
                    continue;
                }
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    if (strcmp(clients[free_index].client_id, "room") == 0) {
        ...
        write(clients[free_index].socketfd, error_message, sizeof(error_message));
        close(clients[free_index].socketfd);

        struct client_data empty;
        clients[free_index] = empty;
        continue;
    }
